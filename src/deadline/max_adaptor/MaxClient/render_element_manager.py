...
class RenderElementManager:
    ...
    def configure_render_elements(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Configure basic settings - support both snake_case and PascalCase
            elements_enabled = (
                self._get_param_value(data, ["RenderElements", "render_elements"], "true").lower()
                == "true"
            )
            ...
            update_paths = (
                self._get_param_value(
                    data, ["RenderElementsUpdatePaths", "render_elements_update_paths"], "true"
                ).lower()
                == "true"
            )
            if update_paths:
    ...
    def _handle_ignore_settings(
        ...
        """
        # Handle ignore by name list - support both naming conventions
        ignore_names_str = self._get_param_value(
            data, ["IgnoreRenderElementsByName", "ignore_render_elements_by_name"], ""
        )
        if ignore_names_str:
            ...
            for element in render_elements:
                ...
                if element_name in ignore_names:
                    ...
                    if element_index >= 0 and self.re_manager is not None:
                        try:
                            # Disable individual render element using the standard 3ds Max API method
                            ...
                            self.logger.error(
                                f"Failed to disable render element '{element_name}': {e}"
                            )
                            # Try alternative approach using MAXScript directly
                            try:
                                rt.execute(
                                    f"renderElementMgr.SetRenderElementEnabled {element_index} false"
                                )
                                self.logger.info(
                                    f"DISABLED render element via MAXScript: '{element_name}' (index {element_index})"
                                )
                                disabled_count += 1
                            except Exception as e2:
                                self.logger.error(
                                    f"Failed to disable render element via MAXScript '{element_name}': {e2}"
                                )
    ...
    def _configure_vray_settings(
        ...
        """
        try:
            # Check if V-Ray VFB control is enabled - support both naming conventions
            vfb_control = (
                self._get_param_value(
                    data,
            ...
            split_buffer = (
                self._get_param_value(
                    data, ["VRaySplitBufferSupport", "vray_split_buffer_support"], "true"
                ).lower()
                == "true"
            )
    ...
    def _get_param_value(
        ...
        """
        Get parameter value with fallback support for different naming conventions.

        Args:
            data: Data dictionary
            param_names: List of parameter names to try (in order of preference)
    ...
    def _convert_data_to_settings(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Convert ignore render elements by name - support both naming conventions
        ignore_names_str = self._get_param_value(
            data, ["IgnoreRenderElementsByName", "ignore_render_elements_by_name"], ""
        )
        if ignore_names_str:
        ...
        # Convert boolean settings - support both naming conventions
