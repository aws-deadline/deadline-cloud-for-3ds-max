...
class _SessionState:
    ...
    don't show it again until 3ds Max is restarted."""
...
def _check_for_update() -> UpdateCheckResult:
    """Check if a newer version of the 3ds Max submitter is available.
    ...
    return safe_check_for_updates(
        integration_name="deadline-cloud-for-3ds-max",
...
def check_and_show_update_dialog() -> bool:
    ...
    If the user previously dismissed the dialog in this 3ds Max session,
    ...
    if update_result.status == UpdateCheckStatus.SUCCESS and update_result.update_available:
        update_dialog = UpdateAvailableDialog(
            ...
            release_notes_url="https://github.com/aws-deadline/deadline-cloud-for-3ds-max/releases",
        )
        update_dialog.exec_()
        if update_dialog.user_downloaded:
            return True
        # User dismissed — suppress for the rest of this 3ds Max session
