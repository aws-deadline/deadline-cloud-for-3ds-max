...
function Build-TestCommand {
    ...
    # Build openjd run command with JSON file references (use filtered parameters for runner)
    $stepName = $selectedStep.name
    $testCommand = "openjd run `"$templateFile`" --step `"$stepName`" --job-param `"file://$runnerJobParamsFile`" --tasks `"file://$taskParamsFile`""
