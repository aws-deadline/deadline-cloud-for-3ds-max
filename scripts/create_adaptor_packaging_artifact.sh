...
# 3ds max is special in that it's product name can't be a module
ADAPTOR_ENTRY=3dsmax-openjd
ADAPTOR_NAME=deadline-cloud-for-3ds-max
MODULE=max
...
if [ "$TAR_BASE" = "" ]; then
    TAR_BASE=$SCRIPTDIR/../$ADAPTOR_ENTRY-py$PYTHON_VERSION-$CONDA_PLATFORM
fi
...
from deadline.${MODULE}_adaptor.${MODULE^}Adaptor.__main__ import main
sys.exit(main(reentry_exe=reentry_exe))
"

cat <<EOF > $BINDIR/$ADAPTOR_ENTRY
...
if [ $CONDA_PLATFORM = "win-64" ]; then
    ...
    cat <<EOF > $BINDIR/$ADAPTOR_ENTRY-script.py
#!C:\\Path\\To\\Python.exe
$PYSCRIPT
EOF
    cp $WORKDIR/tmp/setuptools/cli-64.exe $BINDIR/$ADAPTOR_ENTRY.exe
fi
